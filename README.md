...
Epic 7 的发布前质量门已完成。Epic 1-7 已覆盖核心 MVP 闭环、发布前 QA、安全回归、环境验证和可观测性门禁；当前 dev 环境已完成 2026-05-15 BMAD code review 与自动化测试复验，公开发布仍等待 production configuration gate：

`Launch Readiness QA and Regression Safety Net`

Story 7.4 已把已经积累的 Playwright 回归覆盖、环境变量检查、工具链前置要求和发布前人工判断整理成可重复执行的 launch gate；本地 deterministic `pnpm run qa:launch` 已在 Node `20.20.2` + PostgreSQL 17 + pgvector 环境下通过。`pnpm run deploy:verify:production` 当前仍因生产 secrets 未配置而返回 `NOT READY FOR PRODUCTION LAUNCH`。
...
本地 dev 需要一个可连接的 Postgres，并启用 pgvector。macOS + Homebrew 的最小本地方案：

```bash
brew services start postgresql@17
createuser -s postgres 2>/dev/null || true
createdb -O postgres fracture_recovery 2>/dev/null || true
psql -h localhost -U postgres -d fracture_recovery -c 'CREATE EXTENSION IF NOT EXISTS vector;'
```

如果你使用 Supabase 或其他 Postgres，把下面的 `DATABASE_URL` / `DIRECT_URL` 改成对应连接串即可。
...
然后在 `.env.local` 至少设置这些本地 dev 值：

```bash
NEXT_PUBLIC_APP_URL=http://localhost:3000
NEXTAUTH_URL=http://localhost:3000
NEXTAUTH_SECRET=<use-the-openssl-output-above>
DATABASE_URL=postgresql://postgres:postgres@localhost:5432/fracture_recovery
DIRECT_URL=postgresql://postgres:postgres@localhost:5432/fracture_recovery
EMAIL_SERVER=smtp://user:password@localhost:1025
EMAIL_FROM="Fracture Recovery Companion <hello@example.com>"
```

初始化 Prisma 并启动本地 dev server：
...
本地 deterministic QA 使用 dev-mock Stripe、测试 webhook payload、analytics no-op/mock、no-DSN Sentry 和 deterministic chat provider path；不需要真实 Stripe、Sentry、AI、analytics、Upstash 或邮件发送。
